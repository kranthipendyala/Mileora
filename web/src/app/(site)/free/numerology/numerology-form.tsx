...
export function NumerologyForm() {
  ...
  if (status === "ok") {
    return (
      <div className="mt-6 rounded-xl border border-emerald-400/30 bg-emerald-400/10 p-6 text-center text-emerald-200">
        🙏 Calculated! Your full numerology report is on its way to your phone via WhatsApp.
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="mt-6 grid gap-4">
      <Field label="Full name (as on birth certificate)" error={errors.name?.message}>
      ...
      <label className="flex items-start gap-3 text-sm text-[color:var(--color-text-muted)]">
        ...
        <span>
          I agree to receive my report on WhatsApp and accept the{" "}
      ...
      >
        {status === "submitting" ? "Calculating…" : "Calculate my numbers →"}
      </button>
      {status === "error" && (
        <p className="text-sm text-rose-300">Couldn't process. Please try again.</p>
      )}
